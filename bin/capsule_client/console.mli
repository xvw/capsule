...
(** Perform a coercion to string before calling f. *)
...
(** {3 Timers}
    ...
    - We instantiate a timer with: [Console.time name], where [name] is a unique
      timer ID;
    - [Console.timer_log name] displays the time elapsed since calling
      [Console.timer name];
    - [Console.time_stop name] stops the timer, referenced by its name, in
      progress. *)
...
(** [Console.timetrack name actions] is a shortcut, for example:
    ...
    Where [logger] is a [Console.time_log]. This shortcut avoid the to
    instanciate and closed a timer. *)

(** {3 Groups}

    You can use nested groups to help organize your output by visually combining
    related material. To create a new nested block, call [Console.group ()]. To
    exit the current group, simply call [Console.group_end ()]. *)
