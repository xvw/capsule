...
type t = External of scheme * string | Internal of Yocaml.Path.t
...
let extract_scheme given =
  ...
  let rec aux i acc =
    if i >= len then None
      ...
      match curr with
      | ':' ->
          if i < len - 3 && are_slashes given.[i + 1] given.[i + 2] then
...
let kind_of = function External _ -> "external" | Internal _ -> "internal"
...
let make_urls = function
  | External (s, suffix) -> (suffix, scheme_to_prefix s ^ suffix)
  | Internal p ->
