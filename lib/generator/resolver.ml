module Make (R : Intf.RESOLVABLE) = struct
  ...
  module Source = struct
    ...
    let readings = Path.(specifics / "readings.md")
    let readings_list = Path.(content / "readings.yml")
    let books = Path.(content / "books.yml")
  ...
  module Target = struct
    ...
    let readings = Path.(R.target / "readings.html")
